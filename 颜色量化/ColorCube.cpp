	...
	case 0:
	{
		...
		for (size_t b = bBegin; b <= bEnd; b++)
		{
			...
			if (curSize >= size / 2)
			{
				// 产生新的空间，计算位置和数量
				// 修改本空间的B结束位置和数量
				ColorCube newCube(b, gBegin, rBegin, bEnd, gEnd, rEnd);
				bEnd = b - 1;
				if (b == bBegin) {
					newCube.bBegin = b + 1;
					bEnd = b;
	...
	case 1:
	{
		...
		for (size_t g = gBegin; g <= gEnd; g++)
		{
			...
			if (curSize >= size / 2)
			{
				// 产生新的空间，计算位置和数量
				// 修改本空间的B结束位置和数量
				ColorCube newCube(bBegin, g, rBegin, bEnd, gEnd, rEnd);
				gEnd = g - 1;
				if (g == gBegin) {
					newCube.gBegin = g + 1;
					gEnd = g;
	...
	case 2:
	{
		...
		for (size_t r = rBegin; r <= rEnd; r++)
		{
			...
			if (curSize >= size / 2)
			{
				// 产生新的空间，计算位置和数量
				// 修改本空间的B结束位置和数量
				ColorCube newCube(bBegin, gBegin, r, bEnd, gEnd, rEnd);
				rEnd = r - 1;
				if (r == rBegin) {
					newCube.rBegin = r + 1;
					rEnd = r;
...
void thatboy::ColorCube::splitCube(size_t cubeNumber)
{
    while (colorCubeSet.size() < cubeNumber) {
		...
		if (iter == colorCubeSet.rend())	// 找不到可以分裂的空间
			break;
       // std::max_element(colorCubeSet.begin(), colorCubeSet.end())->splitBrotherCube();// 速度较慢
    }
	colorCubeSet.sort();

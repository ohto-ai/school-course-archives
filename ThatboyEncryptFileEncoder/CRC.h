...
namespace thatboy
{
    ...
    public:
         ...
         * @para nBytes, 数据的长度
         */
        TYPE crcCompute(const unsigned char* message, unsigned int nBytes);
        TYPE crcCompute(const unsigned char* message, unsigned int nBytes, bool reinit);
    protected:
        TYPE m_polynomial;
...
template <typename TYPE>
thatboy::CRC<TYPE>::CRC(TYPE polynomial, TYPE init_remainder, TYPE final_xor_value)
{
    m_width = 8 * sizeof(TYPE);
    m_topbit = 1 << (m_width - 1);
...
template <typename TYPE>
void thatboy::CRC<TYPE>::build(TYPE polynomial, TYPE init_remainder, TYPE final_xor_value)
{
    m_polynomial = polynomial;
    m_initial_remainder = init_remainder;
    m_final_xor_value = final_xor_value;
...
template <typename TYPE>
TYPE thatboy::CRC<TYPE>::crcCompute(const unsigned char* message, unsigned int nBytes)
{
    unsigned int offset;
    unsigned char byte;
...
template <typename TYPE>
TYPE thatboy::CRC<TYPE>::crcCompute(const unsigned char* message, unsigned int nBytes, bool reinit)
{
    unsigned int offset;
    unsigned char byte;
    if (reinit)
